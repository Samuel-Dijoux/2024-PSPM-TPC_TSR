######################################
#  PSPM_TPC-TSR_Thermal-Mismatch_model.R - :
#
...
#     1 : Total ingestion of the resource R by the species 1
...
DefaultParameters <- c(
  Temperature = 20,# Water temperature
  Rmax  = 5.47E-5,   # Resource carying capacity
  ...
  BR    = 0.003,  # Birth rate
  Muo   = 0.005,  # Background mortality rate
  ...
  sw_C_TPC = 1,   # Switching command to implement the temperature-dependence in consumer rates. Binary (on/off): (0) temperature-independent rates or (1) temperature-dependent rates
  sw_P_TPC = 1,   # Switching command to implement the temperature-dependence in predator rates. Binary (on/off): (0) temperature-independent rates or (1) temperature-dependent rates
  sw_C_TNS = 0,   # Switching command to implement a thermal niche shift in consumer species, relative to predator thermal niche. Binary (on/off): (0) no shift or (1) shift relative to predator
  sw_P_TNS = 1,   # Switching command to implement a thermal niche shift in predator species, relative to consumer thermal niche. Binary (on/off): (0) no shift or (1) shift relative to consumer
  Delta_TPC = 1   # Shift in temperature of species thermal niche (consumer or predator) relative to the other species (predator or consumer) [30]
  )
...
LifeHistoryRates <- function(lifestage, istate, birthstate, BirthStateNr, E, pars) {
  with(as.list(c(E, pars, istate)),{
    ...
    # Updated temperatures in species thermal boundaries and optimum depending on the thermal niche shift of either consumer or predator 
    C_added_T = ifelse(sw_C_TNS==1, Delta_TPC, 0);
    P_added_T = ifelse(sw_P_TNS==1, Delta_TPC, 0);
    
    # Temperature-dependent functions of biological rates
    Tlim_C = ifelse( ( (Tlr+C_added_T) > Temperature) || (Temperature > (Tur+C_added_T) ), 0, 1);
    Tlim_P = ifelse( ( (Tlr+P_added_T) > Temperature) || (Temperature > (Tur+P_added_T) ), 0, 1);
    
    T_hump_rate_C = ifelse(Tlim_C == 0, 0, ( (Temperature-(Tlr+C_added_T))*(Temperature-(Tur+C_added_T))/( (Temperature-(Tlr+C_added_T))*(Temperature-(Tur+C_added_T))-(Temperature-(Tor+C_added_T))^2)) );
    T_hump_rate_P = ifelse(Tlim_P == 0, 0, ( (Temperature-(Tlr+P_added_T))*(Temperature-(Tur+P_added_T))/( (Temperature-(Tlr+P_added_T))*(Temperature-(Tlr+P_added_T))-(Temperature-(Tor+P_added_T))^2)) );
    T_expo_rate_C = exp( -Eax*( ((Tor+C_added_T)+T0K)-(Temperature+T0K) )/( Boltz*((Tor+C_added_T)+T0K)*(Temperature+T0K)) );
    
    T_Gr = GR*T_hump_rate_C; GrT = ifelse(sw_C_TPC == 0, GR, T_Gr);
    T_Br = BR*T_hump_rate_C; BrT = ifelse(sw_C_TPC == 0, BR, T_Br);
    T_Ir = IR*T_hump_rate_C; IrT = ifelse(sw_C_TPC == 0, IR, T_Ir);
    T_mu_C = Muo*T_expo_rate_C; MrT_C = ifelse(sw_C_TPC == 0, 2*Muo, Muo+T_mu_C);
    
    FrT = ifelse(sw_P_TPC == 0, 1, T_hump_rate_P);
    ...
    list(
      ...
      impact      = switch(lifestage,
                           c(IrT*Rlim*Length^2, a*Length^b, 0, 0),
                           c(IrT*Rlim*Length^2, 0, a*Length^b, 0),
                           c(IrT*Rlim*Length^2, 0, 0, a*Length^b))
    )
  })
}
...
EnvEqui <- function(I, E, pars) {
  with(as.list(c(E, pars)),{
    ...
    T_hump_rate_P = ifelse(Tlim_P == 0, 0, ( (Temperature-(Tlr+P_added_T))*(Temperature-(Tlr+P_added_T))/( (Temperature-(Tlr+P_added_T))*(Temperature-(Tlr+P_added_T))-(Temperature-(Tor+P_added_T))^2)) );
    T_expo_rate_P = exp( -Eax*( ((Tor+P_added_T)+T0K)-(Temperature+T0K) )/( Boltz*((Tor+P_added_T)+T0K)*(Temperature+T0K)) );
    
    FrT = ifelse(sw_P_TPC == 0, 1, T_hump_rate_P);
    T_mu_P = Muo*T_expo_rate_P; MrT_P = ifelse(sw_P_TPC == 0, 2*Muo, Muo+T_mu_P);
    
    c(Rho*(Rmax - R) - I[1],
      ( Eps*(Ap*I[2]/(1+Ap*Hp*I[2]))*FrT - MrT_P ),
      I[2])
  })
}