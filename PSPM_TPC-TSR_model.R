...
DefaultParameters <- c(
  Temperature = 1,# Water temperature
  Rmax  = 1E-4,   # Resource carying capacity
  ...
  Tsr_lv = -0.00, # Slope of size reduction over temperature per degree celsius for Lv
  ...
  sw_C_BrT = 1,   # Switching command for temperature-dependence in consumer birth rate. Binary (on/off): (0) temperature-independent or (1) temperature-dependent
  sw_C_GrT = 1,   # Switching command for temperature-dependence in consumer growth rate. Binary (on/off): (0) temperature-independent or (1) temperature-dependent
  sw_C_IrT = 1,   # Switching command for temperature-dependence in consumer ingestion rate. Binary (on/off): (0) temperature-independent or (1) temperature-dependent
  sw_C_MuT = 1,   # Switching command for temperature-dependence in consumer mortality rate. Binary (on/off): (0) temperature-independent or (1) temperature-dependent
...
EnvEqui <- function(I, E, pars) {
  with(as.list(c(E, pars)),{
    ...
    Tlim = ifelse( (Tlr > Temperature) || (Temperature > Tur), 0, 1);
    T_hump_rate = ifelse(Tlim == 0, 0, ( (Temperature-Tlr)*(Temperature-Tur)/( (Temperature-Tlr)*(Temperature-Tur)-(Temperature-Tor)^2)) );
    FrT = ifelse(sw_P_FrT == 0, 1, T_hump_rate);
    T_expo_rate = exp( -Eax*(TN-(Temperature+T0K))/(Boltz*TN*(Temperature+T0K)) );
    MrT_P = ifelse(sw_P_MuT == 0, Mup+Muo, Muo*T_expo_rate);
